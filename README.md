...
### 2. SceneLightController - Inspector enum으로 명시적 지정

씬에 하나만 배치하면 되며, `sceneType`과 `lightData`만 지정하면 됩니다.
...
각 씬의 루트 오브젝트에 `SceneLightController`를 붙이고
`sceneType`과 `lightData`를 지정합니다.

```
[SceneLightController]
  sceneType : Primary   // 또는 Additive
...
└─ SceneLightController.cs
    ├─ SceneType (enum)          Primary / Additive Inspector 지정
    ├─ Awake()                   sceneType 기준으로 Manager에 등록
